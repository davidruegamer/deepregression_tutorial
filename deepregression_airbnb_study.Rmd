...
avg_nll <- mean(unlist(lapply(mems, function(x) - mean(as.matrix(tfd_log_prob(x, y))[obs_idx]))))
ens_nll <- - mean(as.matrix(tfd_log_prob(ensd, y))[obs_idx])

matplot(ys, mem_preds, type = "l", lty = 2, col = "gray80", xlab = "log price",
        ylab = "density")
abline(v = y[obs_idx], col = "cornflowerblue")
lines(ys, ens_preds, lwd = 1)
...
matplot(lons, mem_preds, lty = 2, lwd = 1, col = "gray80", type = "l",
        xlab = "longitude", ylab = "Expected log price")
lines(lons, ens_preds)
lines(lons, ens_preds - unc)
lines(lons, ens_preds + unc)
legend("top", c("Individual prediction", "Ensemble prediction"),
       col = c("gray80", "black"), lty = c(2, 1), lwd = 2, bty = "n")
```
